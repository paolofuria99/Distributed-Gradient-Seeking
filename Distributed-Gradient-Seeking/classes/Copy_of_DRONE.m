classdef DRONE < matlab.mixin.Copyable
    ...
    methods(Access=public)
        function obj = DRONE(q, id, type, params)
            ...
            switch obj.type
                case 'unicycle'
                    ...
                    % obj.Q = zeros(length(q));
                    % obj.Q = (rand(length(q),length(q))-0.5)*params.std_dyn_xy;
                    % obj.Q = obj.Q*obj.Q';
        ...
        function u = compute_control(obj,x,index)
            ...
            u = 0.01*direction./direction_norm;
            u = u';

            %% Method 2)
            % desired_distance = 10;
            % direction = [x(1,index)-obj.q_real(1), x(2,index)-obj.q_real(2), 0];
            % direction_norm = norm(direction);
            % error_distance = direction_norm - desired_distance;
            % u = (direction./direction_norm).*error_distance;
            % u = u';

            %% Method 3)
            % Max allowable velocity based on distance wrt final point
            % dt = [1;1e-1;1e-3];
            % direction = [x(1)-obj.q_real(1), x(2)-obj.q_real(2), 0];
            % direction_norm = norm(direction);
            % if direction_norm >= 50
            %     ddt = dt(1)/obj.params.dt;
            %     u = ddt*direction./direction_norm;
            %     u = u';
            % elseif (direction_norm < 50) && (direction_norm >= 20)
            %     ddt = dt(2)/obj.params.dt;
            %     u = ddt*direction./direction_norm;
            %     u = u';
            % elseif (direction_norm < 20) && (direction_norm >= 10)
            %     ddt = dt(3)/obj.params.dt;
            %     u = ddt*direction./direction_norm;
            %     u = u';
            % else
            %     u = [0 0 0];
            ...
            %% Method 4)
            % direction = [x(1,index-1)-p(1), x(2,index-1)-p(2), 0];
            % direction_norm = norm(direction);
            ...
            %% Method 5)
            % direction_dr = [x(1,index)-obj.q_real(1), x(2,index)-obj.q_real(2), 0]; % to get close
            % direction_rd = [obj.q_real(1)-x(1,index), obj.q_real(2)-x(2,index), 0]; % to move away
            % distance = sqrt(x(1,index)-obj.q_real(1))^2+((x(2,index)-obj.q_real(2))^2);
            % if distance <= 10
            %     u = 0.1*direction_rd./distance;
            %     u = u';
            % else
            %     u = 0.1*direction_dr;
            %     u = u';
